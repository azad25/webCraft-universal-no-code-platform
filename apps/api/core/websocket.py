...
class WebSocketManager:
    ...
    def __init__(self):
        # Local connections (per instance)
    ...
    async def disconnect(self, connection_id: str) -> None:
        ...
        # Remove from Redis
        await redis_client.hdel("ws:connections", connection_id)
    ...
    async def join_room(self, connection_id: str, room: str) -> None:
        ...
        # Store in Redis
        await redis_client.sadd(f"ws:room:{room}:members", connection_id)
    ...
    async def leave_room(self, connection_id: str, room: str) -> None:
        ...
        if room in self._rooms:
            ...
            if not self._rooms[room]:
                ...
                await self._pubsub.unsubscribe(f"ws:room:{room}")
        
        conn_info.rooms.discard(room)
        
        # Remove from Redis
        await redis_client.client.srem(f"ws:room:{room}:members", connection_id)
    ...
    async def broadcast_all(
        ...
        # Publish to Redis for other instances
        await redis_client.publish("ws:broadcast", message.to_json())
    ...
    async def _route_message(
        ...
        if msg_type == WSMessageType.PING:
        ...
        elif msg_type == WSMessageType.CURSOR_MOVED:
            ...
            WSMessageType.ELEMENT_ADDED,
            WSMessageType.ELEMENT_UPDATED,
            WSMessageType.ELEMENT_DELETED,
            WSMessageType.ELEMENT_MOVED,
            WSMessageType.ELEMENT_RESIZED
        ]:
            # Broadcast element changes to room
            if message.room:
                await self.broadcast_to_room(
                    message.room,
                    message,
        ...
        elif msg_type == WSMessageType.PRESENCE_UPDATE:
