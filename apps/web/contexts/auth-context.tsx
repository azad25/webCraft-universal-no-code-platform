...
export function AuthProvider({ children }: AuthProviderProps) {
  ...
  const getGoogleAuthUrl = useCallback(async () => {
    const response = await fetch(`/api/v2/auth/oauth/google/url`)
  ...
  const getAppleAuthUrl = useCallback(async () => {
    const response = await fetch(`/api/v2/auth/oauth/apple/url`)
    const data = await response.json()
    localStorage.setItem('oauth_state', data.state)
    return data.authorization_url
  }, [])
  ...
  const getGitHubAuthUrl = useCallback(async () => {
    const response = await fetch(`/api/v2/auth/oauth/github/url`)
    const data = await response.json()
    localStorage.setItem('oauth_state', data.state)
    return data.authorization_url
  }, [])
